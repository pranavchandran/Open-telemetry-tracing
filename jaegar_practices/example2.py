...
def booking_mgr(movie):
    with tracer.start_span('Starting') as span:
        span.set_tag('first', movie)
...
def started():
    with tracer.start_span('Second Starter', child_of=get_current_span()) as span:
        with span_in_context(span):
            num = random.randint(1,10)
            span.set_tag('second', num)
            

# def check_cinema(movie):
#     with tracer.start_span('Checkfirst', child_of=get_current_span()) as span:
#         with span_in_context(span):
